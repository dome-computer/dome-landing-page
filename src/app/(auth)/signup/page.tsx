...
import { novaaDesktopAuthClient } from "@/lib/client";
...
const SignUpComponent = React.memo(
  ({
    searchParams,
  }: {
    searchParams: Promise<{ rs?: string; scope?: string }>;
  }) => {
    // 1. Define your form.
    ...
    const onSubmit = React.useCallback(
      async (values: z.infer<typeof formSchema>) => {
        ...
        const { data } = await novaaDesktopAuthClient.signUp.email(
          {
    ...
    const handle_signup_with_google = React.useCallback(async () => {
      if (
        search_params.rs &&
        search_params.rs == "novaa.desktop" &&
        search_params.scope
      ) {
    ...
    return (
      <div className="w-full h-[100vh] flex items-center justify-center">
        <div className=" shadow-lg rounded-[16px] p-4 w-[90%] md:w-[80%] lg:w-[40%] lg:max-w-[700px] max-h-[80vh] space-y-6 flex flex-col">
          <div className="text-3xl font-bold text-neutral-800 text-center px-2 dark:text-neutral-200">
            Sign Up
          </div>
          <div className="flex flex-col">
            <Button
              onClick={handle_signup_with_google}
