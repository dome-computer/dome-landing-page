...
export const POST = async (req: Request) => {
    ...
    const matches = await conversation_table.find({
        $or: [
            {"chats.content": {$regex: req_data.get("search_value"), $options: "i"}},
            {"chats.title": {$regex: req_data.get("search_value"), $options: "i"}},
        ]
    }).toArray()
